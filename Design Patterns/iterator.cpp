...
class StudentGradesInfoPrinter {
    StudentGradesInfo &my_student;
    int cur_pos = 0;
private:

public:
    StudentGradesInfoPrinter(StudentGradesInfo &student) :
            my_student(student) {
    }
    // reset iterator
    void Reset() {
    ...
    bool HasNext() const {
        ...
        return cur_pos< my_student.GetTotalCoursesCount();
    }
    pair<string, double> GetNext() {
        pair<string, double> result;
        my_student.GetCourseGradeInfo(cur_pos++, result);
        return result;
    }
};
/*
